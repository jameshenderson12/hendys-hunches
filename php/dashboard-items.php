...
function displayLatestInformation() {
	...
	print("<p><strong>Knockout fixtures:</strong><br>Points for predictions of all knockout fixtures will be awarded against the match result after 90 minutes only (not for extra time or penalties). There is now also limited opportunities to view opponents' predictions in a bid to keep competition fresh until the very final game!</p>");
	
	print("<p><strong>Feedback opportunity:</strong><br>Have your say on Hendy's Hunches! I'd be grateful if you could spare 2 minutes to <a href='https://www.surveymonkey.co.uk/r/BQP9FGQ'>complete this quick survey</a>.</p>");
	
	print("<p><strong>Prize information:</strong></p>");
	print("<p></p>");
	print("<ul type='none'>");
	print("<li><img src='img/gold_ros.png' height='20px' alt='Gold rosette' />1st place takes £80</li>");
	print("<li><img src='img/silver_ros.png' height='20px' alt='Silver rosette' />2nd place takes £50</li>");
	print("<li><img src='img/bronze_ros.png' height='20px' alt='Bronze rosette' />3rd place takes £30</li>");
	print("</ul>");
	print("<p>The prizes will be split if any of these spots are shared.</p>");	
...
function displayCharityInformation() {
	...
	printf("<p>Visit the Ballboys website for more information on the charity: <a href='http://ballboys.org.uk/' target='_blank' title='Ballboys charity website'>ballboys.org.uk</a></p>");
	printf("<p><strong>Hendy's Hunches donation:</strong> £%d.00 (40&#37; from entry fees)", $donation);
	printf("<p>Want to give more? Please <a href='https://www.justgiving.com/fundraising/JamesHenderson12' target='_blank' title='Ballboys JustGiving link'>see here</a>.</p>");	
	// Free result set
...
function displayPersonalInfo() {	
	...
	// Get team information from the DB	counting occurrences too
	$sql_getprofileinfo1 = "SELECT avatar, faveteam, fieldofwork, tournwinner, signupdate, haspaid, currpos, prevrank FROM live_user_information WHERE username = '".$_SESSION["username"]."'";
...
//		printf("<li><span class='glyphicon glyphicon-calendar' aria-hidden='true'></span>&nbsp;&nbsp;Signed up on %s</li>", $convertedDate);
		printf("<li><span class='glyphicon glyphicon-pushpin' aria-hidden='true'></span>&nbsp;&nbsp;Placed %s in 2014</li>", $prevrank);
		print("</ul>");
