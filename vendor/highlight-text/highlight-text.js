...
var searchCountID = document.getElementById('searchCount');
...
var highlightClass = "highlightjs",
  ...
  spaceRe2 = /\s+$/,
  before =
    '<span class="' + highlightClass + '" style="display: inline-block; background-color: yellow; color: black; font-weight: bold; font-size: inherit;">',
  after = "</span>",
  // before =
  //   '<span style="display: inline-block; position: relative;"><span class="' +
  //   highlightClass +
  //   '" style="display: block; background: yellow; box-shadow: 0px 0px 1px rgb(120,120,120);">',
  // after = "</span></span>",
  contentError =
    "The ID selector '" +
    targetID +
    "' does not exist in the DOM. Please define a valid ID selector in the user settings of the script and reload the page.";
...
function highlightProcess(tag, content, term) {
  ...
  if (term.length < 3) {
    ...
    if (start.length !== 0) {
      ...
      if (searchCountID) {
        searchCountID.innerHTML = tabIdx.length + " item(s) found";
      }
    }
  } // if else END
} // function highl END
...
/**
 * Function to count the number of highlighted terms and display the count in the specified <div> element
 */
function countAndDisplaySearchTerms() {
  // Get the number of highlighted terms
  var highlightedElements = document.querySelectorAll('.highlightjs');
  var searchTermCount = highlightedElements.length;
  
  // Display the count in the specified <div> element
  var searchCountDiv = document.getElementById('searchCount');
  if (searchCountDiv) {
      searchCountDiv.textContent = "Number of search terms highlighted: " + searchTermCount;
  }
}
...
function createSearchField(inputID) {
   ...
   */
  var dftStyle =
    ".default-wrap{box-sizing:border-box;display:block;position:absolute;width:100%;height:40px;top:0;left:0;box-shadow: 0px 0px 10px rgb(150,150,150);}.input-field-default{box-sizing:border-box;display:block;position:absolute;width:100%;height:40px;top:0;left:0;z-index:1500;padding: 0 20px;border-radius:0px;outline:0;border:none;}";
